...
\begin{document}
	...
	In this report, a detailed analysis of the LIBS (Laser-Induced Breakdown Spectroscopy) spectra for the samples provided by the LSA is presented. The study begins with the visualization of the spectra and an examination of their intrinsic properties, including peak intensities and corresponding wavelengths. In the next step, they are compared with the spectra generated using the NIST LIBS tool. Following this comparative analysis, general considerations for employing these spectra in training machine learning (ML) algorithms for steel grade classification are discussed. The report concludes with a set of open questions and points for clarification that need to be addressed.
	
	The data is provided in two categories: One folder contains the LIBS readings for standard calibration grades (samples).  For each sample there are 5–6 LIBS readings. The table below shows the specifications provided by the LSA for the calibration grades:
...
Figure \ref{fig: 469} presents all LIBS readings for standard grade 469 in a single plot. From this figure, it appears that some of the readings are saturated. To allow for clearer observation, all individual readings, along with the corresponding NIST LIBS spectrum for the same sample, are plotted separately in Figure \ref{fig:469-libs-NIST}.
	...
	\label{fig: 469}
\end{figure}
Based on the observations from Figures \ref{fig:469} and \ref{fig:469-NIST}, it is evident that the spectral structure and observed emission patterns for the same element (in this case, sample 469) differ significantly from the reference spectra provided by the NIST LIBS database. This discrepancy suggests that the intensities captured using a practical, experimental LIBS device can deviate substantially from those generated synthetically using theoretical models based on the Saha–Boltzmann equations. Such differences may arise from various experimental factors, including instrumental sensitivity, plasma temperature fluctuations, matrix effects, and deviations from local thermodynamic equilibrium (LTE).
\begin{figure}[htbp]
	\centering
...
\section{Analyzing the data} \label{h:Probability distributions}
