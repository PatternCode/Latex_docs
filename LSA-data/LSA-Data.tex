...
\begin{document}
	...
	This report presents a detailed analysis of real LIBS data provided by the LSA. The study begins with the visualization of the spectra and an examination of their intrinsic properties, including peak intensities and corresponding wavelengths. These spectra are then compared with those generated using the NIST LIBS tool. Following this comparative analysis, general considerations for using the spectra in training machine learning (ML) algorithms for steel grade classification are discussed. The report concludes with a set of open questions and points requiring further clarification.
...
in case of wavelenths,  also it was observed that the sampling interval for LSA data is not fixed. the intervals range from 0.035 to 0.047 (nm). Figure \ref{fig:Interval_histogram} show the histogram of the intervals for LSA data and Figure \ref{fig:Interval_distribution.png} show the distribution of sampling interval in the whole spectral range (in this case from 246.018(nm) to 414.849(nm)). 
